...
class BSTNode:
    ...
    def recursive_bft(self, level=0):
        ...
        if self.left:
            queue.append(self.left.value)
        if self.right:
            queue.append(self.right.value)

        if self.left:
            queue.extend(self.left.recursive_bft(level=level+1))
        if self.right:
            queue.extend(self.right.recursive_bft(level=level+1))
